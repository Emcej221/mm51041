document.addEventListener("DOMContentLoaded", function () {
  ...
  myLocationBtn.addEventListener("click", function () {
    if (navigator.geolocation) {
      ...
      alert("Geolocation API nie jest wspierane przez twoją przeglądarkę.");
    }
  });
  ...
  function generatePuzzleTiles() {
    ...
    puzzlePreviewContainer.addEventListener("dragstart", function (event) {
      // Zablokuj zdarzenie dla kontenera podglądu puzzli, żeby nie wpływało na pojedyncze kafla puzzla
      if (event.target.classList.contains("puzzle-tile")) {
  ...
  puzzleBtn.addEventListener("click", function () {
    ...
    puzzleContainer.addEventListener("drop", function (event) {
      ...
      // Check if the event target is a child of puzzlePreviewContainer before removing it
      if (puzzlePreviewContainer.contains(event.target)) {
